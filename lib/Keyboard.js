...
export default class extends React.Component {
    ...
    handleChange(event, input) {
        if(!input && event && event.target && event.target.value)
    ...
    if (this.props.options.type == 'textarea') {
         var element = ( <textarea ref="keyboard" value = { this.state.value }  {...other}/> );
     }
     else {
        var element = ( <input ref="keyboard" value = { this.state.value }  {...other} /> );
     }
